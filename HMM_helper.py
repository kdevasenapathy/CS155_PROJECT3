...
def animate_emission(hmm, obs_map, M=8, height=12, width=12, delay=1):
    ...
    def animate(i):
        if i >= delay:
            ...
            if i == 0:
                arrows[int(states[0])][int(states[0])].set_color('red')
            elif i == 1:
                arrows[int(states[0])][int(states[0])].set_color((1 - hmm.A[int(states[0])][int(states[0])], ) * 3)
                arrows[int(states[i - 1])][int(states[i])].set_color('red')
            else:
                arrows[int(states[i - 2])][int(states[i - 1])].set_color((1 - hmm.A[int(states[i - 2])][int(states[i - 1])], ) * 3)
                arrows[int(states[i - 1])][int(states[i])].set_color('red')
